defmodule NoosphericalWeb.CommentLive.Show do
  use Phoenix.LiveComponent
  use Phoenix.HTML

  alias Noospherical.Articles
  alias NoosphericalWeb.CommentLive
  alias NoosphericalWeb.CommentView

  def render(assigns) do
    ~L"""
    <article class="white-text comment comment--<%= @kind %>" id="comment-<%= @id %>">
      ...
      <div class="comment-footer">
        ...
        <p class="comment-permalink">
          <%= link CommentView.format_time(@comment.inserted_at), to: "#comment#{@id}", class: "red-text" %>
        </p>
      </div>
      <%= if @form_visible do %>
        <%= f = form_for @changeset, "#",
          ...
          <div class="comment_form-footer">
            <button class="btn red darken-2" type="submit">Reply</button>
          </div>
        </form>
      <% end %>
  ...
  def mount(socket) do
    {:ok, assign(socket, form_visible: false, changeset: Articles.change_comment()),
  ...
  def preload(list_of_assigns) do
    ...
    children = Articles.fetch_child_comments(parent_ids)
